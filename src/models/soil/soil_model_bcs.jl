...
"""
SoilHeatFlux(value = var(:soil_heat_flux, Ground(Top()), u"W/m^2"); kwargs...) = (internal_energy = (top = FluxBoundaryCondition(value; kwargs...),),)
...
"""
GeothermalHeatFlux(value = var(:geothermal_heat_flux, Ground(Bottom()), u"W/m^2"); kwargs...) = (internal_energy = (bottom = FluxBoundaryCondition(value; kwargs...),),)

"""
Alias for `ValueBoundaryCondition` on top `temperature` (in °C) with the given variable name.
"""
PrescribedSurfaceTemperature(name::Symbol, value = var(name, Ground(Top()), u"°C"); kwargs...) = (temperature = (top = ValueBoundaryCondition(value; kwargs...),),)

"""
Alias for `ValueBoundaryCondition` on top `temperature` (in °C) with the given variable name.
"""
PrescribedBottomTemperature(name::Symbol, value = var(name, Ground(Bottom()), u"°C"); kwargs...) = (temperature = (bottom = ValueBoundaryCondition(value; kwargs...),),)
...
"""
InfiltrationFlux(value = var(:infiltration, Ground(Top()), u"m/s"); kwargs...) = (saturation_water_ice = (top = FluxBoundaryCondition(value; kwargs...),),)
