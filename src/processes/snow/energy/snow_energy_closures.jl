...
@propagate_inbounds function energy_to_temperature!(
    ...
    out.snow_liquid_fraction[i, j, end] = liq * (W_snow > 0)
    ...
    out.snow_temperature[i, j, end] = min(T, zero(T))
    return nothing
end
...
@propagate_inbounds function temperature_to_energy!(
    ...
    out.snow_liquid_fraction[i, j, end] = liq
    C_snow = compute_snow_volumetric_heat_capacity(snow, constants, ρ_snow, liq)
    U_snow = T * C_snow - ρLθ * (one(liq) - liq)
    out.snow_energy[i, j, end] = U_snow * d_snow # integrate by d_snow
    return nothing
end
